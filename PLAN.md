...
- [x] **4. Items as selectable UI.** Render parsed items as a mobile-friendly list of pickable
      components showing name and price.
      *Done:* after parsing, items display; tapping one toggles its selected state.

- [x] **5. Sessions + QR.** Creator calls `POST /api/sessions` (stores the parsed receipt,
...
- [x] **6. Join flow.** Visiting `/s/{id}` lets a guest enter a name and join the session; the
...
- [x] **7. Real-time pick & lock.** Wire Socket.IO (custom Next.js server): picking an item
...
- [x] **8. Split calculation incl. service fee.** Compute each participant's total = their
...
- [x] **9. Summary screen.** A per-session summary listing each participant and what they owe.
